  ...
  try {
    ...
    connection.query(deviceIdQuery, [userId], (err, deviceIdResults) => {
      ...
      connection.query(userQuery, [currentDeviceId], (err, userResults) => {
        ...
        // Fetch CPU wattage from database
        const cpuQuery = 'SELECT avg_watt_usage FROM cpus WHERE model = ?';
        connection.query(cpuQuery, [cpu], (err, cpuResults) => {
          if (err) {
            console.error('Error fetching CPU wattage:', err);
            return res.status(500).json({ error: 'Error fetching CPU wattage data' });
          }
          ...
          // Fetch GPU wattage from database
          const gpuQuery = 'SELECT avg_watt_usage FROM gpus WHERE model = ?';
          connection.query(gpuQuery, [gpu], (err, gpuResults) => {
            if (err) {
              console.error('Error fetching GPU wattage:', err);
              return res.status(500).json({ error: 'Error fetching GPU wattage data' });
            }
            ...
            // Fetch RAM wattage from database
            ...
            connection.query(ramQuery, [ram], (err, ramResults) => {
              if (err) {
                ...
                return res.status(500).json({ error: 'Error fetching RAM wattage data' });
              }
              ...
              connection.query(updateProjectQuery, [carbonEmissions, projectId, userId, userId], (err, results) => {
                ...
                console.log('Carbon emissions calculated successfully:', carbonEmissions);
