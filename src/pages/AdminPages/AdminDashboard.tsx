...
const fetchProjectMembers = async (projectId: number) => {
  try {
    ...
    const response = await fetch(`https://localhost:5000/project_members/${projectId}`, {
      headers: {
...
const checkAndUpdateProjectStatus = async (projectId: number, 
  ...
  try {
    ...
    if (allCompleted) {
      ...
      const updateResponse = await fetch(`https://localhost:5000/admin/update_project/${projectId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      try {
        ...
        const response = await fetch('https://localhost:5000/all_user_projects_admin', {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://localhost:5000/admin/delete_project/${selectedProject.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleSaveEdit = async () => {
    if (selectedProject) {
      try {
        ...
        const response = await fetch(`https://localhost:5000/admin/update_project/${selectedProject.id}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      const createTempUserResponse = await fetch('https://localhost:5000/create_temp_user', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (newProject.project_leader && newProject.project_leader.trim()) {
        try {
          ...
          const leaderResponse = await fetch(`https://localhost:5000/user_organization/${newProject.project_leader}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
      ...
      const response = await fetch('https://localhost:5000/admin/create_project', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const validateEmail = async (email: string) => {
    try {
      ...
      const response = await fetch(`https://localhost:5000/validate_user_email/${email}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleAddAssignee = async () => {
    if (selectedProject && assigneeEmail) {
      try {
        ...
        const response = await fetch('https://localhost:5000/add_project_member', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
