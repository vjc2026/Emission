...
const fetchProjectMembers = async (projectId: number) => {
  try {
    ...
    const response = await fetch(`http://emission-811s.vercel.app/project_members/${projectId}`, {
      headers: {
...
const checkAndUpdateProjectStatus = async (projectId: number, 
  ...
  try {
    ...
    if (allCompleted) {
      ...
      const updateResponse = await fetch(`http://emission-811s.vercel.app/admin/update_project/${projectId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      try {
        ...
        const response = await fetch('http://emission-811s.vercel.app/all_user_projects_admin', {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://emission-811s.vercel.app/admin/delete_project/${selectedProject.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleSaveEdit = async () => {
    if (selectedProject) {
      try {
        ...
        const response = await fetch(`http://emission-811s.vercel.app/admin/update_project/${selectedProject.id}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      const createTempUserResponse = await fetch('http://emission-811s.vercel.app/create_temp_user', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (newProject.project_leader && newProject.project_leader.trim()) {
        try {
          ...
          const leaderResponse = await fetch(`http://emission-811s.vercel.app/user_organization/${newProject.project_leader}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
      ...
      const response = await fetch('http://emission-811s.vercel.app/admin/create_project', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const validateEmail = async (email: string) => {
    try {
      ...
      const response = await fetch(`http://emission-811s.vercel.app/validate_user_email/${email}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleAddAssignee = async () => {
    if (selectedProject && assigneeEmail) {
      try {
        ...
        const response = await fetch('http://emission-811s.vercel.app/add_project_member', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
