...
export function HistoryComponent() {
  ...
  const startSession = async () => {
    ...
    try {
       const response = await fetch(`http://emission-811s.vercel.app/find_project`, {
          method: 'POST',
          headers: {
...
useEffect(() => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await fetch(`http://emission-811s.vercel.app/update_project/${editableProject.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleCompleteStage = async (projectId: number) => {
    ...
    try {
      const response = await fetch(`http://emission-811s.vercel.app/complete_project/${projectId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
