...
const History = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        const response = await fetch('http://emission-811s.vercel.app/user', {
          method: 'GET',
      ...
      // Fetch device type
      try {
        const deviceResponse = await fetch('http://emission-811s.vercel.app/checkDeviceType', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
    ...
    if (task.isRunning) {
      ...
      try {
        ...
        const emissionsEndpoint = currentDevice === 'Laptop' 
          ? 'http://emission-811s.vercel.app/calculate_emissionsM'
          : 'http://emission-811s.vercel.app/calculate_emissions';
        ...
        const membersResponse = await fetch(`http://emission-811s.vercel.app/project/${taskId}/members`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
        ...
        const updateResponse = await fetch('http://emission-811s.vercel.app/user_Update', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const handleComplete = async (taskId: number) => {
    ...
    try {
      ...
      const completeResponse = await fetch(`http://emission-811s.vercel.app/complete_project/${taskId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleProjectRequest = async () => {
    ...
    try {
      ...
      const checkResponse = await fetch('http://emission-811s.vercel.app/check_existing_projectname', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      const response = await fetch('http://emission-811s.vercel.app/user_history', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleAddAssignee = async () => {
    ...
    try {
      if (selectedTask) {
        ...
        const response = await fetch('http://emission-811s.vercel.app/send-invitation', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const handleEditTask = async () => {
    ...
    try {
      ...
      const updateResponse = await fetch(`http://emission-811s.vercel.app/update_project/${selectedTask.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
