...
const History = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      // Fetch device type
      try {
        const deviceResponse = await fetch('https://localhost:5000/checkDeviceType', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
    ...
    try {
      ...
      if (response.ok) {
        ...
        const tasksWithMembers = await Promise.all(data.projects.map(async (project: any) => {
          const membersResponse = await fetch(`https://localhost:5000/project/${project.id}/members`, {
            headers: { 'Authorization': `Bearer ${token}` },
          });
  ...
  const handleTimer = async (taskId: number) => {
    ...
    if (task.isRunning) {
      ...
      try {
        ...
        const membersResponse = await fetch(`https://localhost:5000/project/${taskId}/members`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  const handleComplete = async (taskId: number) => {
    ...
    try {
      ...
      const completeResponse = await fetch(`https://localhost:5000/complete_project/${taskId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleEditTask = async () => {
    ...
    try {
      ...
      const updateResponse = await fetch(`https://localhost:5000/update_project/${selectedTask.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
