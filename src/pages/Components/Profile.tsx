...
export function HELPComponent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const endpoint = deviceType === 'Laptop'
          ? 'https://emission-mah2.onrender.com/displayuserM'
          : 'https://emission-mah2.onrender.com/displayuser';
    ...
    const fetchUserProjects = async (_email: string) => {
      const token = localStorage.getItem('token');
      try {
        const response = await fetch('https://emission-mah2.onrender.com/profile_display_projects', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        const cpuResponse = await axios.get(`https://emission-mah2.onrender.com/${cpuEndpoint}`);
        ...
        const gpuResponse = await axios.get(`https://emission-mah2.onrender.com/${gpuEndpoint}`);
        setGpuOptions(gpuResponse.data.gpuOptions.map((gpu: { label: string; value: string }) => ({
          label: gpu.label,
          value: gpu.value
        })));
  
        const ramResponse = await axios.get('https://emission-mah2.onrender.com/ram-options');
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      const token = localStorage.getItem('token');
      try {
        const response = await fetch('https://emission-mah2.onrender.com/user_devices', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  const handleAddDevice = async () => {
    try {
      ...
      const response = await axios.post('https://emission-mah2.onrender.com/addDevice', formData, {
        headers: {
  ...
  const handleDeviceSwitch = async (deviceId: number) => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch('https://emission-mah2.onrender.com/setCurrentDevice', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
          ...
          <Stack gap="lg">
            ...
            <Button
              ...
              onClick={async () => {
                const token = localStorage.getItem('token');
                try {
                  const response = await fetch(`https://emission-mah2.onrender.com/archive_project/${selectedProject.id}`, {
                    method: 'PUT',
                    headers: { Authorization: `Bearer ${token}` },
                  });
                  if (response.ok) {
