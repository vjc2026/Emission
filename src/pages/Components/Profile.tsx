...
export function HELPComponent() {
  ...
  useEffect(() => {
    ...
    const fetchUserProjects = async (_email: string) => {
      const token = localStorage.getItem('token');
      try {
        const response = await fetch('https://emissionserver.vercel.app/profile_display_projects', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        const cpuResponse = await axios.get(`https://emissionserver.vercel.app/${cpuEndpoint}`);
        ...
        const gpuResponse = await axios.get(`https://emissionserver.vercel.app/${gpuEndpoint}`);
        setGpuOptions(gpuResponse.data.gpuOptions.map((gpu: { label: string; value: string }) => ({
          label: gpu.label,
          value: gpu.value
        })));
  
        const ramResponse = await axios.get('https://emissionserver.vercel.app/ram-options');
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      const token = localStorage.getItem('token');
      try {
        const response = await fetch('https://emissionserver.vercel.app/user_devices', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  const handleAddDevice = async () => {
    try {
      ...
      const response = await axios.post('https://emissionserver.vercel.app/addDevice', formData, {
        headers: {
  ...
  const handleDeviceSwitch = async (deviceId: number) => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch('https://emissionserver.vercel.app/setCurrentDevice', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
          ...
          <Stack gap="lg">
            ...
            <Button
              ...
              onClick={async () => {
                const token = localStorage.getItem('token');
                try {
                  const response = await fetch(`https://emissionserver.vercel.app/archive_project/${selectedProject.id}`, {
                    method: 'PUT',
                    headers: { Authorization: `Bearer ${token}` },
                  });
                  if (response.ok) {
