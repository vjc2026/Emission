...
export function HELPComponent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const endpoint = deviceType === 'Laptop'
          ? 'https://localhost:5000/displayuserM'
          : 'https://localhost:5000/displayuser';
    ...
    const fetchUserProjects = async (_email: string) => {
      const token = localStorage.getItem('token');
      try {
        const response = await fetch('https://localhost:5000/profile_display_projects', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        const cpuResponse = await axios.get(`https://localhost:5000/${cpuEndpoint}`);
        ...
        const gpuResponse = await axios.get(`https://localhost:5000/${gpuEndpoint}`);
        setGpuOptions(gpuResponse.data.gpuOptions.map((gpu: { label: string; value: string }) => ({
          label: gpu.label,
          value: gpu.value
        })));
  
        const ramResponse = await axios.get('https://localhost:5000/ram-options');
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      const token = localStorage.getItem('token');
      try {
        const response = await fetch('https://localhost:5000/user_devices', {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  const handleAddDevice = async () => {
    try {
      ...
      const response = await axios.post('https://localhost:5000/addDevice', formData, {
        headers: {
  ...
  const handleDeviceSwitch = async (deviceId: number) => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch('https://localhost:5000/setCurrentDevice', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
          ...
          <Stack gap="lg">
            ...
            <Button
              ...
              onClick={async () => {
                const token = localStorage.getItem('token');
                try {
                  const response = await fetch(`https://localhost:5000/archive_project/${selectedProject.id}`, {
                    method: 'PUT',
                    headers: { Authorization: `Bearer ${token}` },
                  });
                  if (response.ok) {
